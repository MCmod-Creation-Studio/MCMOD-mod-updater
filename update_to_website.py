...
def login():
    try:
        ...
        while drive.current_url.startswith("https://center.mcmod.cn/") is False and drive.find_elements(By.CLASS_NAME,'header-user-avatar') == []:
            ...
            try:
                with open(os.path.join(upload_folder, path), 'r', encoding='utf-8') as file:
                    ...
                    try:
                        ...
                        time.sleep(1)
                        print(os.path.abspath(os.path.join(upload_folder,filename)))
                        pyautogui.write(os.path.abspath(os.path.join(upload_folder,filename)))
                        time.sleep(0.5)
                        pyautogui.press('enter')
                        time.sleep(0.5)
                        pyautogui.press('enter')
                        time.sleep(0.5)
                        fill_mod_detail(file)
                        time.sleep(5)
...
def fill_mod_detail(file):
    ...
    if any([platform_tick, function_tick, tag_tick]):
        print("自动化操作完成！")
    else:
